...
from services.apps_engine import build_secrets, database_provider_capabilities, secret_vault, snapshots
...
def _string_map(raw: Any, label: str, *, allowed: set[str] | None = None, normalize_keys: bool = False) -> dict[str, str]:
    if raw in (None, ""):
        return {}
    ...
    for key, value in raw.items():
        clean_key = build_secrets.normalize_environment_key(key) if normalize_keys else str(key)
        if allowed is not None and clean_key not in allowed:
            continue
        if not clean_key:
            continue
        clean_val = build_secrets.normalize_environment_value(value) if normalize_keys else (str(value) if value is not None else "")
...
async def _deploy_app(
    ...
    app = await container_app_service.create_app(
        ...
        internal_port=port,
        ssl_requested=ssl_requested and not has_certificate,
        environment_values=_string_map(payload.get("environment_values"), "Environment values", normalize_keys=True),
