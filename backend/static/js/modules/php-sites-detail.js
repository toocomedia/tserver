...
function exactConfirmation(text) { return window.prompt(`${t("type_to_confirm")}\n${text}`) === text; }
...
async function handle(action, trigger) {
  ...
  if (action === "db-delete") { const db = site.database.database; if (!exactConfirmation(`DELETE DATABASE ${db}`)) return; payload = await request(actionUrl(id, "/database"), "DELETE", { confirmation: `DELETE DATABASE ${db}` }); }
  ...
  if (action === "ssl-revoke") { if (!exactConfirmation(`REVOKE ${site.domain}`)) return; payload = await request(actionUrl(id, "/ssl"), "DELETE", { confirmation: `REVOKE ${site.domain}` }); }
  ...
  if (action === "delete-site") { if (!exactConfirmation(`DELETE ${site.domain}`)) return; payload = await request(actionUrl(id), "DELETE", { confirmation: `DELETE ${site.domain}`, delete_database: window.confirm(t("delete_database_too")) }); window.location.assign("/php-sites/"); return; }
...
root?.addEventListener("click", async (event) => { const trigger = event.target.closest("[data-action]"); if (!trigger) return; trigger.disabled = true; setError(""); try { await handle(trigger.dataset.action, trigger); } catch (err) { setError(err.message); trigger.disabled = false; } });
if (root) load();
