...
class AppsEngineSafeDeploymentTests(unittest.TestCase):
    ...
    def test_setup_handoff_requires_a_reviewed_plan_and_never_unlocks_deployment_secrets(self):
        ...
        self.assertIn("setup_documentation_failed", chat)
        self.assertIn("ask one concise [INPUT:] or [OPTION:] question", chat)
        ...
        self.assertIn("Apply Plan", markdown)
        ...
        self.assertIn("applyAiAppPlan", actions)
        self.assertIn("/plugins/ai_helper/api/action-plans/", actions)
