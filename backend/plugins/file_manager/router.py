...
from plugins.file_manager import audit, file_service
...
async def apps(db: AsyncSession = Depends(get_db)):
    records = list((await db.scalars(select(ContainerApp).where(
        ContainerApp.status == "running",
    ).order_by(ContainerApp.id.desc()))).all())
    domain_ids = [item.domain_id for item in records]
    domains = (await db.scalars(select(Domain).where(Domain.id.in_(domain_ids)))).all() if domain_ids else []
    names = {item.id: item.name for item in domains}
    return {"apps": [{
        "id": item.id,
        "domain": names.get(item.domain_id),
        "preset": item.preset,
        "status": item.status,
    } for item in records]}
...
async def roots(app_id: int, db: AsyncSession = Depends(get_db)):
    return {"app_id": app_id, "roots": await file_service.roots_for(db, app_id)}


@router.get("/api/apps/{app_id}/roots/{root_id}/entries")
async def entries(
    app_id: int, root_id: str, request: Request, path: str = Query(""),
    db: AsyncSession = Depends(get_db),
):
    return await _operation(request, db, app_id, root_id, path, "list", lambda context: file_service.list_entries(context, path))


@router.get("/api/apps/{app_id}/roots/{root_id}/text")
async def read_text(
    app_id: int, root_id: str, request: Request, path: str,
    db: AsyncSession = Depends(get_db),
):
    return await _operation(request, db, app_id, root_id, path, "read", lambda context: file_service.read_text(context, path))


@router.post("/api/apps/{app_id}/roots/{root_id}/text")
async def write_text(
    app_id: int, root_id: str, body: TextWrite, request: Request,
    db: AsyncSession = Depends(get_db),
):
...
async def create_directory(
    app_id: int, root_id: str, body: DirectoryCreate, request: Request,
    db: AsyncSession = Depends(get_db),
):
...
async def upload(
    app_id: int, root_id: str, request: Request, path: str = Form(...),
    ...
    try:
        return await _operation(request, db, app_id, root_id, path, "upload", lambda context: {
            "size": file_service.write_upload_file(context, path, temporary, size, etag)
        }, size_bytes=size)
    finally:
...
async def download(
    ...
    staged, size = await _operation(request, db, app_id, root_id, path, "download", lambda context: file_service.stage_download(context, path))
    return FileResponse(
...
async def move(
    app_id: int, root_id: str, body: Transfer, request: Request,
    db: AsyncSession = Depends(get_db),
):
...
async def delete(
    app_id: int, root_id: str, body: DeleteRequest, request: Request,
    db: AsyncSession = Depends(get_db),
):
...
async def _operation(
    request: Request, db: AsyncSession, app_id: int, root_id: str, path: str,
    action: str, operation: Callable[[file_service.FileContext], Any], *, size_bytes: int | None = None,
):
    safe_path = _audit_path(path)
    async with file_service.lock_for(app_id):
        try:
            context = await file_service.resolve_context(db, app_id, root_id)
            result = await asyncio.to_thread(operation, context)
        except HTTPException:
            await audit.record(db, request, app_id=app_id, root_id=root_id, relative_path=safe_path, action=action, result="failed", size_bytes=size_bytes)
            await db.commit()
            raise
    ...
    await audit.record(db, request, app_id=app_id, root_id=root_id, relative_path=safe_path, action=action, result="success", size_bytes=recorded_size, item_count=count)
    return result if result is not None else {"ok": True}


async def _protected_environment_keys(db: AsyncSession, app_id: int, root_id: str) -> set[str]:
    if root_id != "runtime-env":
        return set()
...
def _mkdir(context: file_service.FileContext, path: str) -> dict[str, bool]:
    file_service.create_directory(context, path)
    return {"ok": True}


def _write(context: file_service.FileContext, body: TextWrite, protected: set[str]) -> dict[str, Any]:
    return {
        "size": file_service.write_text(context, body.path, body.content, body.etag, protected),
...
def _transfer(context: file_service.FileContext, body: Transfer, *, copy: bool) -> dict[str, bool]:
    file_service.move_or_copy(context, body.source_path, body.destination_path, copy=copy)
    return {"ok": True}


def _delete(context: file_service.FileContext, path: str) -> dict[str, bool]:
    file_service.delete_path(context, path)
    return {"ok": True}
