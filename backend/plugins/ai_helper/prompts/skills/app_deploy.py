...
2. Multi-Container / Stacks: When an application requires cooperating services (e.g. web app + database + analytics engine/cache, such as Plausible, Ghost + MySQL, Nextcloud, etc.), analyze the architecture and immediately invoke `propose_official_stack_install` in your tool calls. Provide the dynamic `services` dictionary with full registry image coordinates (e.g. 'ghcr.io/plausible/community-edition:v3.2.1', 'postgres:16-alpine', 'clickhouse/clickhouse-server:24.12-alpine'), internal ports, volumes, health checks, `startup_order`, `web_service_name`, `web_internal_port`, `required_secrets` (key and purpose), `url_templates` (with {service_name} and {SECRET_KEY} placeholders), and `default_environment`. You must execute the `propose_official_stack_install` tool call in the same response.
