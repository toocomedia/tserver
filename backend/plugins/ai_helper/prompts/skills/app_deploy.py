...
3. Make exactly one review plan:
   - Single app: `propose_app_install` with the web port, non-secret environment values, panel database attachments, storage mounts, SecretSpecs, and a verified or standard health path. When `official_image_recommendation` is detected in inspection, prioritize the recommended official Docker image and port, explaining the recommendation clearly.
