...
5. If inspection shows Compose services or unsupported single-app datastores such as ClickHouse, do not create a single-app plan; create a restricted stack plan or report that stack evidence is insufficient.
6. Health: use an HTTP path only when source evidence proves it. Unknown means disabled, not `/health` or `/api/health`.
7. Secrets: name/purpose/generator only. The server generates and binds values after the user clicks Deploy reviewed setup. External connection URLs are user-entered only. Never emit a credential-unlock action tag: generated deployment secrets are never readable in chat.
