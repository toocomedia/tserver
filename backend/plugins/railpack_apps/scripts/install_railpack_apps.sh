...
if ! docker inspect --format '{{.State.Running}}' srv-panel-buildkit 2>/dev/null | grep -qx true || ! docker inspect --format '{{.HostConfig.ExtraHosts}}' srv-panel-buildkit 2>/dev/null | grep -q "host.docker.internal"; then
...
if ! docker buildx inspect srv-panel-builder >/dev/null 2>&1; then
  docker buildx create --name srv-panel-builder --driver docker-container --driver-opt default-load=true --driver-opt extra-hosts=host.docker.internal:host-gateway >/dev/null 2>&1 || true
fi
railpack --version
